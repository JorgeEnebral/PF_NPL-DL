...
def train_step_ner(model, train_data, loss_fn_ner, optimizer, writer, epoch, device):
    ...
    print(f"Epoch {epoch}: Train Loss NER = {np.mean(losses):.4f} | Train Accuracy = {acc_val:.4f}")


@torch.enable_grad()
def train_step_nersa(model, train_data, loss_fn_ner, loss_fn_sa, opt_ner, opt_sa, writer, epoch, device):
    model.train()
    ...
    print(f"Epoch {epoch}: Train Loss NER = {np.mean(losses_ner):.4f} | Accuracy NER = {acc_ner_val:.4f}")
...
def val_step_ner(model, val_data, loss_fn_ner, writer, epoch, device):
    ...
    print(f"Epoch {epoch}: Validation Loss NER = {np.mean(losses):.4f} | Accuracy = {acc_val:.4f}")
...
def val_step_nersa(model, val_data, loss_fn_ner, loss_fn_sa, writer, epoch, device):
    ...
    print(f"Epoch {epoch}: Validation Loss NER = {np.mean(losses_ner):.4f} | Accuracy NER = {acc_ner_val:.4f}")
